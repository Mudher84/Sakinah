...
public final class PrayerRefreshWorker extends Worker {
    ...
    private static final int OFFLINE_DAYS=7;
    ...
    @NonNull @Override public Result doWork(){
        ...
        try{
            ...
            int method=bridge.optInt("method",4),school=bridge.optInt("school",0);
            JSONObject cfg=bridge.optJSONObject("notifications");if(cfg==null)cfg=new JSONObject();
            ZoneId zone;try{zone=ZoneId.of(bridge.optString("timezone",ZoneId.systemDefault().getId()));}catch(Exception e){zone=ZoneId.systemDefault();}
            LocalDate today=LocalDate.now(zone);
            JSONArray cache=new JSONArray();
            for(int i=0;i<OFFLINE_DAYS;i++){
                JSONObject data=refreshDate(today.plusDays(i),lat,lon,method,school,cfg,i==0);
                JSONObject day=new JSONObject();
                day.put("date",today.plusDays(i).toString());
                day.put("timings",data.optJSONObject("timings"));
                day.put("calendar",data.optJSONObject("date"));
                day.put("meta",data.optJSONObject("meta"));
                cache.put(day);
            }
            prefs.edit().putString("offline_prayer_days",cache.toString()).putLong("last_refresh_success",System.currentTimeMillis()).putString("last_refresh_error","").apply();
            SakinahWidgetProvider.refreshAll(context);
            return Result.success();
        }catch(Exception e){prefs.edit().putString("last_refresh_error",e.getClass().getSimpleName()).apply();return Result.retry();}
    }

    private JSONObject refreshDate(LocalDate date,double lat,double lon,int method,int school,JSONObject cfg,boolean isToday)throws Exception{
        Context context=getApplicationContext();
        JSONObject data=fetch(date,lat,lon,method,school),timings=data.getJSONObject("timings");
        ...
        String dateKey=date.toString();long now=System.currentTimeMillis(),nextAt=Long.MAX_VALUE;String nextPrayer="",nextTime="";
        boolean pre=cfg.optBoolean("prePrayer",true);int preMinutes=Math.max(1,Math.min(120,cfg.optInt("prePrayerMinutes",10)));

        for(String p:PRAYERS){
            String alarmId=p+"@"+dateKey,preId="PrePrayer@"+p+"@"+dateKey;
            if(!cfg.optBoolean(p,true)){
                PrayerScheduler.cancel(context,alarmId);PrayerScheduler.cancel(context,preId);continue;
            }
            String value=clean(timings.optString(p,""));if(value.isBlank())continue;
            long at=toEpoch(date,value,zone);
            if(at>now+30000L){
                PrayerScheduler.schedule(context,alarmId,p,at,"حان وقت صلاة "+ar(p));
                if(pre){long preAt=at-preMinutes*60_000L;if(preAt>now+30000L)PrayerScheduler.schedule(context,preId,"PrePrayer",preAt,"تبقى "+preMinutes+" دقيقة على صلاة "+ar(p));else PrayerScheduler.cancel(context,preId);}else PrayerScheduler.cancel(context,preId);
        ...
        if(shouldUpdate){SharedPreferences.Editor e=p.edit().putString("next_prayer",nextPrayer).putString("next_prayer_time",nextTime).putLong("next_prayer_at",nextAt);if(hijri!=null){String day=hijri.optString("day","");JSONObject month=hijri.optJSONObject("month");String monthAr=month==null?"":month.optString("ar",month.optString("en",""));String year=hijri.optString("year","");e.putString("hijri_date",(day+" "+monthAr+" "+year).trim());}e.apply();}
        return data;
    }
    ...
    private JSONObject fetch(LocalDate date,double lat,double lon,int method,int school)throws Exception{
        ...
        HttpURLConnection c=(HttpURLConnection)new URL(endpoint).openConnection();c.setConnectTimeout(12000);c.setReadTimeout(12000);c.setRequestProperty("Accept","application/json");
        ...
        JSONObject data=new JSONObject(s.toString()).optJSONObject("data");if(data==null)throw new IllegalStateException("No data");return data;
    }
    private static String enc(String s){return URLEncoder.encode(s,StandardCharsets.UTF_8);}
    private static String clean(String s){int i=s.indexOf(' ');return(i>=0?s.substring(0,i):s).trim();}
    private static long toEpoch(LocalDate date,String hhmm,ZoneId zone){String[] a=hhmm.split(":");return ZonedDateTime.of(date,LocalTime.of(Integer.parseInt(a[0]),Integer.parseInt(a[1])),zone).toInstant().toEpochMilli();}
