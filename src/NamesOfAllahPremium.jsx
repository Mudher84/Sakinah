...
export default function NamesOfAllahPremium({go}){
 ...
 const detailSheet=selected?<div className="np-overlay" onClick={()=>setSelected(null)}><div className="np-sheet" onClick={e=>e.stopPropagation()}><div className="np-sheetbar"><button className="np-iconbtn" onClick={()=>setSelected(null)}>✕</button><span style={{fontFamily:'IBM Plex Mono',fontSize:10,color:C.gold2}}>{selected.num} / ٩٩</span></div><div className="np-sheetname">{selected.ar}</div><div className="np-latin" style={{color:C.soft}}>{selected.latin}</div><div className="np-box"><div style={{fontSize:10,color:C.gold2}}>المعنى</div><div style={{fontSize:12,lineHeight:1.8,marginTop:6}}>{selected.full||selected.meaning}</div></div>{selected.proof&&<div className="np-box"><div style={{fontSize:10,color:C.gold2}}>من الدليل</div><div className="np-proof">{selected.proof}</div><div style={{fontSize:9.5,color:C.soft,textAlign:'center',marginTop:6}}>{selected.ref}</div></div>}<div className="np-actions"><button className="np-primary" style={{background:C.navy,color:C.bg}} onClick={()=>toggleSaved(selected)}>{saved.has(selected.num)?'محفوظ':'أضف إلى المحفوظات'}</button></div></div></div>:null;
 return <div className="names-premium" dir="rtl">
  <style>{`
   ...
   .np-overlay{position:fixed;inset:0;z-index:2147483647;background:rgba(16,45,67,.45);display:flex;align-items:flex-end;justify-content:center}.np-sheet{width:min(100%,430px);background:${C.bg};border-radius:26px 26px 0 0;padding:19px 20px calc(24px + env(safe-area-inset-bottom,0px));box-shadow:0 -20px 50px -20px rgba(16,45,67,.6);max-height:76vh;overflow:auto}.np-sheetbar{display:flex;justify-content:space-between;align-items:center}.np-sheetname{font-family:'Amiri',serif;font-size:36px;text-align:center;margin-top:8px}.np-box{margin-top:9px;padding:13px 15px;border-radius:16px;background:${C.paper};border:1px solid rgba(16,45,67,.07)}.np-proof{font-family:'Amiri',serif;font-size:18px;line-height:1.9;text-align:center;margin-top:6px}
  `}</style>
