...
function snapshot(){
 ...
 try{
  ...
  if(raw){
   surah=Number(raw.surahId||raw.surah||1)||1;
   ayah=Number(raw.ayah||raw.ayahId||4)||4;
   page=Number(raw.page||12)||12;
   if(raw.savedAt){const m=Math.max(1,Math.round((Date.now()-raw.savedAt)/60000));ago=m<60?`قبل ${arNum(m)} دقيقة`:`قبل ${arNum(Math.max(1,Math.round(m/60)))} ساعة`;}
  }
 }catch{}
 let downloads=0;
...
export default function QuranToolsPremium({go}){
 ...
 return <div className="qtp-stage" dir="rtl">
  <style>{`
   .qtp-stage{position:fixed;inset:0;z-index:2147483500;background:#E6E0D4;display:flex;align-items:center;justify-content:center;overflow:hidden;font-family:'Noto Kufi Arabic','Cairo',sans-serif;color:${C.navy}}
   .qtp-stage *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;scrollbar-width:none}.qtp-stage *::-webkit-scrollbar{width:0;height:0;display:none}
   .qtp-frame{width:min(100%,406px);height:min(100dvh,874px);padding:8px;min-height:0}
   .qtp-shell{width:100%;height:100%;min-height:0;border-radius:34px;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;overscroll-behavior-y:contain;background:${C.bg};box-shadow:0 40px 90px -40px rgba(16,45,67,.5),0 0 0 1px rgba(16,45,67,.08)}
   .qtp-top{position:sticky;top:0;z-index:10;background:rgba(247,243,234,.94);backdrop-filter:blur(10px);border-bottom:1px solid ${C.line};padding:15px 20px;display:flex;align-items:center;justify-content:space-between}
   .qtp-topbtn{width:44px;height:44px;min-width:44px;min-height:44px;border:0;background:transparent;color:#5B6672;font-family:inherit;font-size:18px;cursor:pointer}.qtp-topbtn:last-child{color:#B08D4F;font-size:15px}.qtp-title{font-size:13px;font-weight:600;text-align:center}
   ...
   .qtp-shortcuts{margin:10px 20px 0;display:flex;gap:9px}.qtp-shortcut{flex:1;min-height:64px;padding:13px 8px;border-radius:18px;background:${C.card};border:1px solid ${C.line};text-align:center;color:${C.navy};font-family:inherit;cursor:pointer}.qtp-shortcut i{display:block;font-style:normal;font-size:15px;color:#B08D4F}.qtp-shortcut span{display:block;font-size:11px;font-weight:500;margin-top:6px}
   .qtp-group{margin:24px 20px 0}.qtp-ghead{display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px}.qtp-gtitle{font-size:13px;font-weight:600}.qtp-gnote{font-size:10px;color:#A9B0B8}.qtp-panel{border-radius:22px;background:${C.card};border:1px solid ${C.line};overflow:hidden}.qtp-row{width:100%;display:flex;align-items:center;gap:13px;padding:14px 16px;border:0;border-top:1px solid ${C.line};background:transparent;color:${C.navy};font-family:inherit;text-align:right;cursor:pointer}.qtp-row:first-child{border-top:0}.qtp-row.active{background:rgba(192,160,98,.09)}.qtp-icon{width:36px;height:36px;min-width:36px;border-radius:13px;background:rgba(192,160,98,.13);color:#B08D4F;display:grid;place-items:center;font-size:14px}.qtp-copy{flex:1;min-width:0}.qtp-name{display:block;font-size:13px;font-weight:500}.qtp-desc{display:block;font-size:10px;color:${C.muted};margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.qtp-meta{font-family:'IBM Plex Mono',monospace;font-size:10px;color:#B08D4F;flex:none}.qtp-arrow{font-size:12px;color:${C.soft};flex:none}.qtp-stats{margin:24px 20px 0;padding:16px 18px;border-radius:22px;background:#EFE8DA;border:1px solid rgba(192,160,98,.28);display:flex;align-items:center;justify-content:space-between}.qtp-stat{text-align:center;flex:1}.qtp-statn{font-family:'IBM Plex Mono',monospace;font-size:15px;color:${C.goldDark}}.qtp-statl{font-size:10px;color:#8A7F6C;margin-top:4px}.qtp-bottom{height:118px;min-height:118px}
   @supports (height:env(safe-area-inset-bottom)){.qtp-bottom{height:calc(118px + env(safe-area-inset-bottom));min-height:calc(118px + env(safe-area-inset-bottom))}}
   @media(max-width:405px){.qtp-stage{align-items:stretch;background:${C.bg}}.qtp-frame{width:100%;height:100dvh;padding:0}.qtp-shell{border-radius:0}}
  `}</style>
  <div className="qtp-frame"><main className="qtp-shell">
   ...
   <section className="qtp-stats">{STATS.map(([num,label])=><div className="qtp-stat" key={label}><div className="qtp-statn">{num}</div><div className="qtp-statl">{label}</div></div>)}</section>
   <div className="qtp-bottom" aria-hidden="true"/>
  </main></div>
 </div>;
}
