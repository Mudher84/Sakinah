...
export default function SakinahSevenDock(){
 ...
 return <div className="sakinah-seven-shell" onClickCapture={captureAudioHub} style={{position:"relative",minHeight:"100vh",background:C.ivory,color:C.ink}} dir="rtl">
  <style>{`
   ...
   .sakinah-seven-shell .sakinah-seven-dock{position:fixed!important;left:50%!important;right:auto!important;top:auto!important;transform:translateX(-50%)!important;bottom:max(10px,env(safe-area-inset-bottom))!important;z-index:2147483000!important;width:min(560px,calc(100vw - 14px));display:grid!important;grid-template-columns:repeat(7,minmax(0,1fr));gap:3px;padding:7px;border:1px solid rgba(255,255,255,.24)!important;border-radius:26px;background:linear-gradient(135deg,rgba(255,255,255,.055),rgba(246,243,236,.018))!important;-webkit-backdrop-filter:blur(34px) saturate(185%)!important;backdrop-filter:blur(34px) saturate(185%)!important;box-shadow:0 14px 38px rgba(16,16,15,.055),inset 0 1px 0 rgba(255,255,255,.34),inset 0 -1px 0 rgba(255,255,255,.04)!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;overflow:hidden}
   .sakinah-seven-shell .sakinah-seven-dock:before{content:"";position:absolute;inset:1px;border-radius:25px;pointer-events:none;background:linear-gradient(115deg,rgba(255,255,255,.085),transparent 28%,transparent 72%,rgba(255,255,255,.025));opacity:.82}
   .sakinah-seven-dock button{position:relative;z-index:1;border:0;border-radius:17px;padding:9px 2px;background:transparent;color:rgba(16,16,15,.58);font-family:inherit;display:grid;place-items:center;min-width:0;min-height:54px;transition:transform .22s ease,background .22s ease,color .22s ease,box-shadow .22s ease}
   .sakinah-seven-dock button:active{transform:scale(.92)}
   .sakinah-seven-dock button.active{background:linear-gradient(145deg,rgba(255,255,255,.11),rgba(255,255,255,.035))!important;color:#173B57;box-shadow:inset 0 0 0 1px rgba(255,255,255,.16),0 4px 12px rgba(23,59,87,.035)!important;-webkit-backdrop-filter:blur(9px);backdrop-filter:blur(9px)}
