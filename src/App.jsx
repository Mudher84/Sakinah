...
function DiscoverScreen({ lang, go, isFriday }) {
  ...
  const life = [
    ...
    { l: lang === "ar" ? "القرّاء" : "Reciters", action: () => go("reciters") },
    { l: lang === "ar" ? "الجمعة والعيد" : "Friday & Eid", action: () => go("friday-eid") },
    { l: lang === "ar" ? "أسماء الله الحسنى" : "Names of Allah", action: () => go("names") },
    { l: lang === "ar" ? "المكتبة الإسلامية" : "Islamic Library", action: () => go("islamic-library") },
    { l: lang === "ar" ? "المعلم والإمام" : "Teacher & Imam", action: () => go("teacher-imam") },
    { l: lang === "ar" ? "الرؤية الكاملة" : "Complete Vision", action: () => go("complete-vision") },
  ];
  return (
    <div style={{ position: "absolute", inset: 0, background: COLOR.ivory, color: COLOR.ink, display: "flex", flexDirection: "column" }}>
      ...
      <div className="sk-scroll" style={{ flex: 1, overflowY: "auto", padding: "6px 24px 130px" }}>
        {isFriday ? (
          ...
          <div onClick={() => go("names")} style={{ marginTop: 10, cursor: "pointer" }}>
        ...
        <div onClick={() => go("complete-vision")} style={{ marginTop: 34, border: `1px solid rgba(16,16,15,.10)`, borderRadius: 18, padding: "18px 18px 17px", cursor: "pointer", background: "rgba(255,255,255,.42)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
            <div>
              <div style={{ fontSize:10, letterSpacing:".12em", fontWeight:700, opacity:.42, marginBottom:7 }}>{lang === "ar" ? "RC4 · SHOWCASE" : "RC4 · SHOWCASE"}</div>
              <div className="font-editorial" style={{ fontSize:lang === "ar" ? 23 : 20, lineHeight:1.2 }}>{lang === "ar" ? "كل مميزات سكينة" : "All Sakinah Features"}</div>
              <div style={{ marginTop:7, fontSize:11.5, opacity:.5, lineHeight:1.7, maxWidth:285 }}>{lang === "ar" ? "اعرض كل المميزات التي اتفقنا عليها كنسخة استعراضية، حتى لو لم يتم ربط خدماتها الحقيقية بعد." : "See the complete feature set as a visual showcase, even where production services are not connected yet."}</div>
            </div>
            {lang === "ar" ? <ChevronLeft size={18} opacity={.35} /> : <ChevronRight size={18} opacity={.35} />}
          </div>
        </div>
      </div>
    </div>
...
const EXPANDED_WORLDS = {
  ...
  platform: [
    ...
    ["complete-vision", "الرؤية الكاملة", "Complete Vision"],
  ],
};
    ...
    <EditorialHub lang={lang} go={go} titleAr="الأذان والمؤذنون" titleEn="Adhan & Muezzins"
      ...
      sections={[
        ...
        { id:"behaviour", ar:"سلوك الأذان", en:"BEHAVIOUR", items:[
          ...
          { ar:"فتح إعداد الأذان لكل صلاة", en:"Configure Adhan per prayer", status:"local", go:"adhan-config" },
        ]},
      ]} />
  );
}
  ...
  return <EditorialHub lang={lang} go={go} titleAr="منصة القرآن" titleEn="Quran Platform"
    ...
    sections={[
      ...
      { id:"audio", ar:"الاستماع والحفظ", en:"AUDIO & MEMORIZATION", items:[
        { ar:"مكتبة القرّاء", en:"Reciter library", subAr:"المصدر والترخيص والجودة والتنزيل", subEn:"Source, license, quality and offline state", status:"adapter", go:"reciters" },
        ...
        { ar:"رفيق القرآن الشخصي", en:"Personal Quran Companion", status:"local", go:"quran-companion" },
      ]},
  ...
  return <EditorialHub lang={lang} go={go} titleAr="المصادر والثقة" titleEn="Trust & Sources"
    ...
    sections={[
      ...
      { id:"lifecycle", ar:"دورة المراجعة", en:"REVIEW LIFECYCLE", items:[
        ...
        { ar:"سجل تصحيحات شفاف", en:"Transparent correction history", status:"adapter", go:"corrections" },
      ]},
  ...
  return <EditorialHub lang={lang} go={go} titleAr="دون اتصال والمزامنة" titleEn="Offline & Sync"
    ...
    sections={[
      ...
      { id:"sync", ar:"البيانات الشخصية", en:"PERSONAL DATA", items:[
        ...
        { ar:"وضع الضيف وخصوصية AI", en:"Guest & AI privacy modes", status:"local", go:"guest-privacy" },
        { ar:"مزامنة مشفرة اختيارية", en:"Optional encrypted sync", status:"adapter" },
      ]},
      { id:"recovery", ar:"الاعتمادية", en:"RELIABILITY", items:[
        { ar:"استكمال التنزيل بعد الانقطاع", en:"Download resume/retry", status:"adapter", go:"downloads-manager" },
  ...
  return <EditorialHub lang={lang} go={go} titleAr="التحقق والنزاهة" titleEn="Verification & Integrity"
    ...
    sections={[
      { id:"integrity", ar:"سلامة البيانات", en:"DATA INTEGRITY", featured:true, items:[
        { ar:"توقيع حزم القرآن والحديث والتفسير", en:"Cryptographic signing for religious packs", status:"adapter", go:"corrections" },
  ...
  return <EditorialHub lang={lang} go={go} titleAr="السفر والحرمين" titleEn="Travel & Haramain"
    ...
    sections={[
      ...
      { id:"qibla", ar:"القبلة", en:"QIBLA", items:[
        ...
        { ar:"خريطة القبلة عند ضعف الحساس", en:"Qibla map fallback", status:"device", go:"qibla-map" },
  ...
  return <EditorialHub lang={lang} go={go} titleAr="أجهزة سكينة" titleEn="Sakinah Surfaces"
    ...
    sections={[
      { id:"surfaces", ar:"الواجهات", en:"SURFACES", featured:true, items:[
        ...
        { ar:"Home/Ambient/Public Display", en:"Home/Ambient/Public Display", status:"adapter", go:"public-display" },
...
function CompleteVisionCenter({ lang, go }) {
  const sections = [
    { id:"prayer-production", ar:"الصلاة والقبلة والأذان — إنتاج", en:"PRAYER · QIBLA · ADHAN — PRODUCTION", featured:true, items:[
      { ar:"Prayer Engine: الموقع والمنطقة الزمنية وطريقة الحساب والعصر وخطوط العرض العليا والتعديلات", en:"Prayer Engine: location, timezone, calculation/Asr methods, high-latitude rules and adjustments", status:"adapter", go:"me-prayer-settings" },
      { ar:"Next Prayer موحّد لليوم والصلاة والودجت والتنبيهات مع عبور منتصف الليل وDST", en:"Unified Next Prayer for Today, Prayer, widgets and notifications with midnight/DST handling", status:"adapter", go:"prayer" },
      { ar:"Qibla Engine: geodesic bearing + rotation-vector sensors + calibration + Map fallback + haptic alignment", en:"Qibla Engine: geodesic bearing, rotation-vector sensors, calibration, Map fallback and haptic alignment", status:"device", go:"prayer" },
      { ar:"Adhan Engine مستقل: جدولة بعد reboot، Audio Focus، Bluetooth، المكالمات، DND، شاشة القفل", en:"Independent Adhan Engine: reboot scheduling, audio focus, Bluetooth, calls, DND and lock screen", status:"device", go:"adhan-center" },
      { ar:"مكتبة مؤذنين مرخّصة ومعاينة وتنزيل واختيار مختلف لكل صلاة وإقامة منفصلة", en:"Licensed Muezzin library with preview/download, per-prayer selection and separate Iqamah", status:"adapter", go:"adhan-center" },
      { ar:"Prepare for Prayer: وضوء، قبلة، هدوء تدريجي، وإقامة المسجد عند توفر مصدر", en:"Prepare for Prayer: Wudu, Qibla, gradual quieting and verified mosque Iqamah", status:"device", go:"sacred" },
      { ar:"أوقات خاصة قابلة للتفسير: الشروق والضحى ومنتصف الليل والثلث الأخير عند اعتماد المنهج", en:"Explainable special times: sunrise, Duha, Islamic midnight and last third when methodology is configured", status:"adapter" },
    ]},
    { id:"quran-complete", ar:"القرآن — المنصة الكاملة", en:"QURAN — COMPLETE PLATFORM", items:[
      { ar:"مصحف موثّق: سورة/آية/صفحة/جزء/حزب/ربع/سجدة مع سلامة النص والإصدار", en:"Verified Mushaf: Surah/Ayah/page/Juz/Hizb/Rub/Sajdah with text integrity and versioning", status:"adapter", go:"quran-platform" },
      { ar:"Page Mode موثّق + Reflow Mode لإمكانية الوصول بدون تغيير النص", en:"Authoritative Page Mode + accessible Reflow Mode without altering Quran text", status:"adapter", go:"quran-platform" },
      { ar:"تجويد موثّق، تعلم التلاوة، تسجيل صوتي محلي، تكرار آية/نطاق، إخفاء/إظهار للحفظ", en:"Verified Tajweed, recitation learning, local recording, Ayah/range repeat and hide/reveal memorization", status:"adapter", go:"quran-platform" },
      { ar:"Quran Audio Engine: قراء متعددون، آية/سورة، background، MediaSession، lock screen، headset، sleep timer", en:"Quran Audio Engine: multiple reciters, Ayah/Surah, background, MediaSession, lock screen, headset and sleep timer", status:"device", go:"quran-platform" },
      { ar:"تنزيلات Offline بجودة وحجم واضحين واستكمال/إعادة محاولة وWi‑Fi only", en:"Offline audio downloads with quality/size, resume/retry and Wi-Fi-only controls", status:"adapter", go:"offline-sync" },
      { ar:"الروايات والقراءات مع فصل النص والصوت، ومقارنة التلاوات للمختصين", en:"Riwayat/Qira'at with strict text/audio separation and advanced recitation comparison", status:"adapter", go:"scholar" },
      { ar:"ترجمات متعددة لكل لغة مع المترجم/الجهة/الإصدار ومقارنة الترجمات", en:"Multiple translations per language with translator/provider/version and comparison", status:"adapter" },
      { ar:"الجذر والصرف والعربية القرآنية والمواضع الأخرى للكلمة", en:"Roots, morphology, Quranic Arabic and other occurrences", status:"adapter" },
      { ar:"Quran Journey: قراءة واستماع وحفظ وخطط ختمة مرنة بدون streaks أو ضغط", en:"Quran Journey: reading, listening, memorization and adaptive Khatmah plans without streak pressure", status:"local" },
      { ar:"ملاحظات شخصية وCollections وDeep Links ومشاركة تحفظ المرجع", en:"Private notes, collections, deep links and reference-preserving sharing", status:"local" },
    ]},
    { id:"knowledge-trust", ar:"المعرفة والثقة", en:"KNOWLEDGE & TRUST", items:[
      { ar:"مكتبات تفسير متعددة ومقارنة بدون دمج يوهم بالإجماع", en:"Multiple Tafsir libraries and comparison without synthetic consensus", status:"adapter", go:"tafsir-hadith" },
      { ar:"حديث: مجموعة/كتاب/باب/راوي/مرجع/تصنيف ومصدر التصنيف وبحث وتخريج مساعد", en:"Hadith: collection/book/chapter/narrator/reference/grade/source plus search and assisted lookup", status:"adapter", go:"tafsir-hadith" },
      { ar:"اختلاف فقهي محترم: آراء معترف بها ومصادرها بدون RIGHT/WRONG مصطنع", en:"Respectful scholarly difference with sourced recognized views, never synthetic RIGHT/WRONG", status:"adapter", go:"trust-center" },
      { ar:"Verification Layer لكل ناتج حساس: الصلاة والقبلة والهجري والحديث والتفسير وAI والمسجد", en:"Verification Layer for prayer, Qibla, Hijri, Hadith, Tafsir, AI and mosque data", status:"adapter", go:"trust-center" },
      { ar:"زر لماذا؟ يشرح الحساب/المصدر/درجة الثقة وحالة عدم اليقين", en:"Why? explains calculation, provenance, confidence and uncertainty state", status:"local", go:"trust-center" },
      { ar:"Content lifecycle: Draft → Pending → Reviewed → Verified → Retired + audit/correction history", en:"Content lifecycle: Draft → Pending → Reviewed → Verified → Retired with audit/correction history", status:"adapter", go:"verification" },
      { ar:"توقيع تشفيري للحزم، checksums، rollback، kill switch للحزمة التالفة، وتصحيح عاجل", en:"Cryptographic pack signing, checksums, rollback, bad-pack kill switch and urgent corrections", status:"adapter", go:"verification" },
      { ar:"مكتبة إسلامية: كتب، خطب، دروس، نص/صوت، بحث، ملاحظات، Offline، حقوق واضحة", en:"Islamic library: books, khutbahs, lessons, text/audio, search, notes, offline and explicit rights", status:"adapter", go:"archive" },
      { ar:"Sakinah Archive + Seerah Timeline + History Atlas + Time Machine بمصادر", en:"Sakinah Archive + Seerah Timeline + History Atlas + sourced Time Machine", status:"adapter", go:"archive" },
    ]},
    { id:"worship-life", ar:"العبادة والحياة", en:"WORSHIP & LIFE", items:[
      { ar:"الأذكار: صباح/مساء/بعد الصلاة/نوم/استيقاظ/سفر، مصدر وتكرار ومفضلة", en:"Adhkar: morning/evening/after-prayer/sleep/waking/travel with source, repeat and favorites", status:"adapter", go:"adhkar-home" },
      { ar:"الدعاء: قرآن وسنة بالمصدر + دعاء شخصي محلي منفصل", en:"Du'a: Quran/Sunnah with provenance + separate private personal Du'a", status:"adapter", go:"dua-center" },
      { ar:"أسماء الله الحسنى بمنهج ومصدر لا كـ99 Cards افتراضية", en:"Names of Allah with methodology and sources, not generic 99 cards", status:"adapter" },
      { ar:"الطهارة كاملة: وضوء، غسل، تيمم، نجاسة، مسح، مع الاختلافات عند الحاجة", en:"Complete purification learning: Wudu, Ghusl, Tayammum, impurities and wiping with sourced differences", status:"adapter", go:"learning" },
      { ar:"تعلم الصلاة خطوة بخطوة + Beginner Mode + صوت/Transliteration اختياري", en:"Step-by-step prayer learning + Beginner Mode + optional audio/transliteration", status:"adapter", go:"learning" },
      { ar:"الصيام: رمضان والأيام البيض والاثنين والخميس وعرفة وعاشوراء والست من شوال بدون افتراض أن المستخدم صائم", en:"Fasting contexts without inferring that the user is fasting", status:"adapter", go:"fasting" },
      { ar:"Friday Moment وEid Moment وRamadan Moments وآخر عشر ليالٍ بسياق موثوق", en:"Friday, Eid, Ramadan and last-ten-nights Moments from trusted calendar context", status:"adapter", go:"calendar-zakat" },
      { ar:"الزكاة بحاسبة شفافة للنصاب والمنهج + الصدقة والوقف بجهات موثقة فقط", en:"Transparent Zakat calculator + charity/Waqf only through verified organizations", status:"adapter", go:"finance" },
      { ar:"الميراث والوصية كأدوات تعليمية Rule Engine مدققة مع المذهب والقانون المحلي", en:"Inheritance/will as audited educational rule-engine tools with school/local-law context", status:"adapter", go:"finance" },
      { ar:"Life journeys: زواج، مولود، وفاة/جنازة، مرض، سفر، عمل، مال، دين، صدقة", en:"Life journeys: marriage, newborn, bereavement, illness, travel, work, money, debt and charity", status:"adapter", go:"life-center" },
    ]},
    { id:"mosque-family", ar:"المسجد والعائلة والمجتمع", en:"MOSQUE · FAMILY · COMMUNITY", items:[
      { ar:"My Mosque: أذان/إقامة/جمعة/خطبة/دروس/حلقات/فعاليات/خدمات من حساب مسجد موثّق", en:"My Mosque: Adhan/Iqamah/Friday/khutbah/lessons/circles/events/services from verified mosque identity", status:"adapter", go:"mosque" },
      { ar:"QR رسمي للمسجد وتوثيق الإدارة وإبلاغ عن خطأ في البيانات", en:"Official mosque QR, admin verification and data-error reporting", status:"adapter", go:"mosque" },
      { ar:"Mosque Mode + Screen-off/quiet mode بدون تتبع عبادة", en:"Mosque Mode + screen-off/quiet mode without worship tracking", status:"device", go:"sacred" },
      { ar:"Family Space خاص: خطط قرآن، مناسبات، تعلم أطفال، حج/عمرة بدون مراقبة الصلاة", en:"Private Family Space: Quran plans, events, kids learning and Hajj/Umrah without worship surveillance", status:"adapter", go:"family" },
      { ar:"Sakinah Kids: قرآن، قصص، صلاة، دعاء، عربية، أخلاق، تاريخ؛ بلا إعلانات أو AI مفتوح أو نقاط عبادة", en:"Sakinah Kids: Quran, stories, prayer, Du'a, Arabic, character and history; no ads/open AI/worship points", status:"adapter", go:"kids" },
      { ar:"Teacher Mode / Quran Circle: واجبات وتسجيلات خاصة واختيارية", en:"Teacher Mode / Quran Circle with private optional assignments and recordings", status:"adapter", go:"community" },
      { ar:"Imam/Mosque Mode لإدارة الإقامة والدروس والإعلانات من جهة موثقة", en:"Imam/Mosque Mode for verified Iqamah, lessons and announcements", status:"adapter", go:"mosque" },
      { ar:"Community without Social Media: ختمات ومبادرات وصدقة بلا followers/likes/feed/leaderboards", en:"Community without social media: Khatmah, initiatives and charity without followers/likes/feed/leaderboards", status:"adapter", go:"community" },
    ]},
    { id:"travel-haramain", ar:"السفر والحرمين والحج", en:"TRAVEL · HARAMAIN · HAJJ", items:[
      { ar:"Travel Mode صريح: مدن محفوظة، مواقيت متعددة، تغير timezone، حزمة قبل السفر", en:"Explicit Travel Mode: saved cities, multi-location times, timezone changes and pre-trip pack", status:"device", go:"journey-services" },
      { ar:"Hajj Companion مراحل كاملة + Group Mode + نقاط تجمع + Offline guidance", en:"Hajj Companion with stages, Group Mode, meeting points and offline guidance", status:"adapter", go:"hajj" },
      { ar:"Umrah Companion: إحرام/ميقات/طواف/سعي/إكمال مع مصادر", en:"Umrah Companion: Ihram/Miqat/Tawaf/Sa'i/completion with sources", status:"adapter", go:"umrah" },
      { ar:"Makkah/Madinah contexts: خرائط وخدمات وأبواب ومرافق وصول من مصادر رسمية", en:"Makkah/Madinah contexts: maps, services, gates and accessibility from official sources", status:"adapter", go:"journey-services" },
      { ar:"Emergency Hajj Card + Lost Mode بمشاركة موقع مؤقتة ومشفرة", en:"Emergency Hajj Card + Lost Mode with explicit temporary encrypted location sharing", status:"adapter", go:"journey-services" },
      { ar:"Emergency Offline Pack: قرآن، أدعية، مناسك، خرائط محملة، معلومات الرحلة", en:"Emergency Offline Pack: Quran, Du'a, rituals, downloaded maps and trip information", status:"adapter", go:"offline-sync" },
      { ar:"Flight mode awareness للصلاة/الوقت/الاتجاه بدون إصدار حكم فقهي آلي", en:"Flight awareness for time/prayer/direction without automated religious rulings", status:"device", go:"journey-services" },
    ]},
    { id:"intelligence", ar:"سكينة الذكية", en:"SAKINAH INTELLIGENCE", items:[
      { ar:"Universal Command: أفعال التطبيق أولاً ثم بيانات محلية ثم استرجاع موثوق ثم AI", en:"Universal Command: app actions first, then local data, sourced retrieval, then AI", status:"adapter", go:"intelligence" },
      { ar:"Sakinah Voice: أوامر عربية طبيعية ولهجات للاستماع والقرآن والصلاة والقبلة والبحث", en:"Sakinah Voice: natural Arabic/dialect commands for audio, Quran, prayer, Qibla and search", status:"device", go:"intelligence" },
      { ar:"Sakinah Lens: التعرف على موضع آية مطبوعة وفتحها؛ لا استنباط أحكام من الصور", en:"Sakinah Lens: recognize printed Ayah location and open it; never infer rulings from images", status:"adapter", go:"intelligence" },
      { ar:"Global Search: بدون تشكيل وأخطاء إملائية وجذور ونص/مرجع/موضوع وSemantic labels", en:"Global Search: diacritic-insensitive, typo/root aware, text/reference/topic and labelled semantic results", status:"adapter", go:"search" },
      { ar:"AI citations لكل فقرة + verifier يتأكد أن المصدر يدعم الادعاء + abstention", en:"Per-claim AI citations + citation verifier + abstention when evidence is insufficient", status:"adapter", go:"trust-center" },
      { ar:"AI disagreement mode يعرض اختلاف المصادر المعتبرة بدل دمجها", en:"AI disagreement mode exposes recognized source differences instead of blending them", status:"adapter" },
      { ar:"Local-only / Trusted Cloud / AI Off مع عدم بناء ملف نفسي أو درجة تدين", en:"Local-only / Trusted Cloud / AI Off with no psychological or religiosity profiling", status:"local", go:"me-privacy" },
      { ar:"Quran Companion يتذكر الاستمرار والتفضيلات محلياً بدون chatbot أو guilt", en:"Quran Companion remembers continuity/preferences locally without chatbot or guilt", status:"local", go:"quran-platform" },
    ]},
    { id:"platform", ar:"المنصة والأجهزة", en:"PLATFORM & SURFACES", items:[
      { ar:"Offline-first core بدون حساب: قرآن/موضع/محفوظات/حساب الصلاة/القبلة/أذكار", en:"Accountless offline-first core: Quran/position/bookmarks/prayer math/Qibla/Adhkar", status:"adapter", go:"offline-sync" },
      { ar:"Download Manager موحد: pause/resume/retry/corruption/storage/Wi‑Fi only", en:"Unified Download Manager: pause/resume/retry/corruption/storage/Wi-Fi only", status:"adapter", go:"offline-sync" },
      { ar:"Backup/Export/Delete + مزامنة مشفرة اختيارية أو مزامنة الموضع فقط", en:"Backup/Export/Delete + optional encrypted sync or position-only sync", status:"adapter", go:"offline-sync" },
      { ar:"Widgets: Prayer/Day Light/Quran/Adhkar/Ramadan/Mosque بأحجام متعددة", en:"Widgets: Prayer/Day Light/Quran/Adhkar/Ramadan/Mosque in multiple sizes", status:"device", go:"devices" },
      { ar:"Wear OS: الصلاة والقوس والقبلة والذكر والتحكم بالصوت", en:"Wear OS: prayer, arc, Qibla, Dhikr and audio control", status:"device", go:"devices" },
      { ar:"Car/Android Auto: تجربة صوتية آمنة بدون تشتيت", en:"Car/Android Auto: safe audio-first experience", status:"device", go:"devices" },
      { ar:"Home/Ambient/Public Display للمسجد والبيت + burn-in protection", en:"Home/Ambient/Public Display for mosque/home with burn-in protection", status:"adapter", go:"devices" },
      { ar:"Tablet/Foldable/Desktop multi-window + Scholar workspace + keyboard shortcuts", en:"Tablet/Foldable/Desktop multi-window + Scholar workspace + keyboard shortcuts", status:"device", go:"scholar" },
      { ar:"E-ink / low-end / low-storage / low-bandwidth / Data Saver", en:"E-ink / low-end / low-storage / low-bandwidth / Data Saver", status:"device", go:"global-product" },
      { ar:"دعم أجهزة بدون خدمات Google قدر الإمكان", en:"Avoid mandatory dependence on Google services where practical", status:"adapter", go:"global-product" },
    ]},
    { id:"accessibility-privacy", ar:"إمكانية الوصول والخصوصية", en:"ACCESSIBILITY & PRIVACY", items:[
      { ar:"Simple/Senior Mode: صلاة، قرآن، أذكار، قبلة بأهداف لمس وخط كبير", en:"Simple/Senior Mode: Prayer, Quran, Adhkar and Qibla with large type/targets", status:"local", go:"me-accessibility" },
      { ar:"TalkBack/Screen Reader، high contrast، reduced motion، dynamic type، non-visual Qibla guidance", en:"TalkBack, high contrast, reduced motion, dynamic type and non-visual Qibla guidance", status:"device", go:"me-accessibility" },
      { ar:"دعم المكفوفين/Braille displays حيث تسمح المنصة، ولغة الإشارة للمحتوى التعليمي", en:"Blind/Braille-display support where platform allows, plus sign-language learning content", status:"adapter", go:"learning" },
      { ar:"Guest/Anonymous Mode لا يكشف السجل والعائلة والملاحظات", en:"Guest/Anonymous Mode hides history, family and private notes", status:"local", go:"me-privacy" },
      { ar:"Screenshot privacy للملاحظات والعائلة والتسجيلات الخاصة", en:"Screenshot privacy for notes, family and private recordings", status:"device", go:"me-privacy" },
      { ar:"الموقع والميكروفون والإشعارات بصلاحيات واضحة، لا تتبع دائم", en:"Explicit location/microphone/notification permissions; no continuous tracking", status:"device", go:"me-privacy" },
      { ar:"لا إعلانات تتبع ولا بيع بيانات ولا Analytics للعبادة أو الأسئلة الدينية", en:"No surveillance ads, data selling or analytics on worship/religious queries", status:"local", go:"security-admin" },
    ]},
    { id:"operations-quality", ar:"الجودة والتشغيل العالمي", en:"QUALITY · OPERATIONS · GLOBAL", items:[
      { ar:"Onboarding قصير: لغة، موقع/مدينة، طريقة الصلاة، تفضيلات القرآن؛ بدون إجبار حساب", en:"Short onboarding: language, location/city, prayer method, Quran preferences; no forced account", status:"local", go:"onboarding" },
      { ar:"Migration/import للـBookmarks وموضع القراءة عند توفر تنسيق قابل للنقل", en:"Migration/import for bookmarks and reading position where portable formats exist", status:"adapter", go:"onboarding" },
      { ar:"Localization عالمي: Arabic-first + English + Urdu/Bahasa/Turkish/French وغيرها", en:"Global localization: Arabic-first + English + Urdu/Bahasa/Turkish/French and more", status:"adapter", go:"global-product" },
      { ar:"Regional packs للعراق والخليج وإندونيسيا وتركيا وغيرها بدون Forks", en:"Regional packs for Iraq/Gulf/Indonesia/Turkey and more without forks", status:"adapter", go:"global-product" },
      { ar:"CMS ومراجعون وصلاحيات وتراخيص لكل خط/كتاب/صوت/قارئ/مؤذن", en:"CMS, reviewer roles and licensing for every font/book/audio/reciter/muezzin", status:"adapter", go:"security-admin" },
      { ar:"Telemetry تقني فقط للأعطال والأداء بدون سجل العبادة", en:"Technical crash/performance telemetry only, never worship history", status:"adapter", go:"security-admin" },
      { ar:"Reference QA: بغداد/مكة/لندن/أوسلو/جاكرتا/نيويورك، DST، polar، RTL/LTR", en:"Reference QA across Baghdad/Makkah/London/Oslo/Jakarta/New York, DST, polar and RTL/LTR", status:"adapter", go:"verification" },
      { ar:"Disaster Mode: إذا توقفت السيرفرات يبقى Core المحلي يعمل", en:"Disaster Mode: local core remains functional when servers fail", status:"local", go:"offline-sync" },
      { ar:"شفافية التحديثات الدينية والتصحيحات العامة وعدم إخفاء الأخطاء المهمة", en:"Transparent religious-data updates and public correction history", status:"adapter", go:"verification" },
    ]},
  ];
  return <EditorialHub lang={lang} go={go}
    titleAr="كل مميزات سكينة — RC4"
    titleEn="All Sakinah Features — RC4"
    introAr="نسخة استعراضية كاملة: كل ميزة اتفقنا عليها موجودة هنا حتى نراها داخل التطبيق. لا يعني ظهور الميزة أنها موصولة بخدمة حقيقية بعد؛ هذه المرحلة للواجهة والتجربة البصرية أولاً."
    introEn="Full showcase build: every agreed feature is visible inside the app. Visibility does not imply a live production integration yet; this stage is for product structure and visual experience first."
    sections={sections} />;
}
...
/* ════════════════════════════════════════════════════════════════
   RC3 — INTERACTIVE FEATURE COMPLETION LAYER
   These screens turn previously registered concepts into usable
   prototype interactions. Source/device/backend-dependent behavior
   remains explicitly labelled and is never faked as production.
════════════════════════════════════════════════════════════════ */

function SectionShell({ lang, go, titleAr, titleEn, subtitleAr, subtitleEn, children, back="discover" }) {
  return <div style={{ position:"absolute", inset:0, background:COLOR.ivory, color:COLOR.ink, display:"flex", flexDirection:"column" }}>
    <div style={{ padding:"24px 24px 0" }}><BackBtn lang={lang} onClick={()=>go(back)} /></div>
    <div className="sk-scroll" style={{ flex:1, overflowY:"auto", padding:"16px 24px 130px" }}>
      <div className="font-editorial" style={{ fontSize:lang==="ar"?30:27, lineHeight:1.2 }}>{lang==="ar"?titleAr:titleEn}</div>
      {(subtitleAr||subtitleEn) && <div style={{ fontSize:12.5, opacity:.5, lineHeight:1.75, marginTop:9 }}>{lang==="ar"?subtitleAr:subtitleEn}</div>}
      {children}
    </div>
  </div>;
}
...
function ChoicePill({ active, children, onClick }) {
  return <button onClick={onClick} style={{ border:`1px solid ${active?COLOR.goldDeep:"rgba(16,16,15,.12)"}`, color:active?COLOR.goldDeep:COLOR.ink, background:"none", borderRadius:20, padding:"7px 11px", fontSize:11.5, cursor:"pointer" }}>{children}</button>;
}

function ReciterLibrary({ lang, go }) {
  const reciters=[
    {id:"provider-a", ar:"قارئ — مصدر مرخّص مطلوب", en:"Reciter — licensed provider required", metaAr:"لا يوجد ملف صوت مضمّن في النموذج", metaEn:"No audio file is bundled in this prototype"},
    {id:"provider-b", ar:"قارئ ثانٍ — مصدر موثوق", en:"Second reciter — trusted source", metaAr:"هيكل جاهز للبث والتنزيل", metaEn:"Architecture ready for streaming/download"},
    {id:"local", ar:"ملف محلي يختاره المستخدم", en:"User-selected local file", metaAr:"للتجربة فقط، لا يغيّر هوية النص القرآني", metaEn:"Prototype-only; never changes Quran text identity"},
  ];
  const [selected,setSelected]=useState(reciters[0].id);
  const [playing,setPlaying]=useState(null);
  const [quality,setQuality]=useState("standard");
  return <SectionShell lang={lang} go={go} titleAr="مكتبة القرّاء" titleEn="Reciter Library" subtitleAr="اختيار القارئ والجودة والتنزيل مستقل عن المصحف. لا يتم شحن تسجيلات غير مرخّصة." subtitleEn="Reciter, quality and downloads are separate from Mushaf data. No unlicensed recordings are shipped." back="quran-platform">
    <div style={{ display:"flex", gap:8, marginTop:22, flexWrap:"wrap" }}>
      <ChoicePill active={quality==="standard"} onClick={()=>setQuality("standard")}>{lang==="ar"?"جودة قياسية":"Standard"}</ChoicePill>
      <ChoicePill active={quality==="high"} onClick={()=>setQuality("high")}>{lang==="ar"?"جودة عالية":"High"}</ChoicePill>
      <ChoicePill active={quality==="data"} onClick={()=>setQuality("data")}>{lang==="ar"?"توفير البيانات":"Data Saver"}</ChoicePill>
    </div>
    <div style={{ marginTop:22 }}>
      {reciters.map((r,i)=><div key={r.id} style={{ borderTop:"1px solid rgba(16,16,15,.08)", padding:"15px 0" }}>
        <div style={{ display:"flex", justifyContent:"space-between", gap:14, alignItems:"center" }}>
          <div><div style={{ fontSize:14.5, fontWeight:selected===r.id?600:500 }}>{lang==="ar"?r.ar:r.en}</div><div style={{ fontSize:11, opacity:.45, marginTop:4 }}>{lang==="ar"?r.metaAr:r.metaEn}</div></div>
          <button onClick={()=>setPlaying(playing===r.id?null:r.id)} style={{ border:"none", background:"none", cursor:"pointer", color:COLOR.ink }}>{playing===r.id?<Pause size={17}/>:<Play size={17}/>}</button>
        </div>
        <div style={{ display:"flex", gap:14, marginTop:10 }}>
          <button onClick={()=>setSelected(r.id)} style={{ border:"none", background:"none", padding:0, cursor:"pointer", color:selected===r.id?COLOR.goldDeep:COLOR.ink, fontSize:11.5 }}>{selected===r.id?(lang==="ar"?"القارئ الافتراضي":"Default reciter"):(lang==="ar"?"تعيين افتراضي":"Set default")}</button>
          <button style={{ border:"none", background:"none", padding:0, opacity:.45, fontSize:11.5 }}>{lang==="ar"?"تنزيل — يحتاج مزود":"Download — provider required"}</button>
        </div>
      </div>)}
    </div>
  </SectionShell>;
...
function AdhanConfigurator({ lang, go }) {
  const prayers=["fajr","dhuhr","asr","maghrib","isha"];
  const [sameAll,setSameAll]=useState(true);
  const [mode,setMode]=useState("full");
  const [prefs,setPrefs]=useState(Object.fromEntries(prayers.map(p=>[p,"licensed-a"])));
  const labels={"licensed-a":lang==="ar"?"أذان مرخّص A":"Licensed Adhan A","licensed-b":lang==="ar"?"أذان مرخّص B":"Licensed Adhan B","simple":lang==="ar"?"تنبيه قصير":"Short tone"};
  return <SectionShell lang={lang} go={go} titleAr="إعداد الأذان" titleEn="Adhan Configuration" subtitleAr="كل صلاة مستقلة، مع خيار صوت واحد للجميع. التشغيل الحقيقي بالخلفية يحتاج Android native واختبار جهاز." subtitleEn="Each prayer can be independent, with an all-prayers option. Real background playback requires Android-native implementation and device validation." back="adhan-center">
    <div style={{ marginTop:22, display:"flex", justifyContent:"space-between", alignItems:"center", borderTop:"1px solid rgba(16,16,15,.08)", padding:"15px 0" }}><span style={{fontSize:14}}>{lang==="ar"?"استخدام نفس الأذان للجميع":"Use same Adhan for all"}</span><Switch on={sameAll} onChange={()=>setSameAll(v=>!v)}/></div>
    <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:16 }}>
      {["off","notification","short","full"].map(x=><ChoicePill key={x} active={mode===x} onClick={()=>setMode(x)}>{lang==="ar"?({off:"إيقاف",notification:"إشعار",short:"قصير",full:"أذان كامل"}[x]):({off:"Off",notification:"Notification",short:"Short",full:"Full Adhan"}[x])}</ChoicePill>)}
    </div>
    {prayers.map((p,i)=><div key={p} style={{ borderTop:"1px solid rgba(16,16,15,.08)", padding:"14px 0", opacity:sameAll&&i>0?.45:1 }}>
      <div style={{ display:"flex", justifyContent:"space-between", gap:10, alignItems:"center" }}><span style={{fontSize:14.5,fontWeight:i===0?600:500}}>{NAMES[p][lang]}</span><select disabled={sameAll&&i>0} value={sameAll&&i>0?prefs.fajr:prefs[p]} onChange={e=>setPrefs(s=>({...s,[p]:e.target.value}))} style={{ background:"transparent", border:"1px solid rgba(16,16,15,.15)", borderRadius:8, padding:"6px 8px", fontFamily:"inherit" }}>{Object.entries(labels).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></div>
    </div>)}
    <div style={{ marginTop:20, fontSize:11.5, opacity:.48, lineHeight:1.65 }}>{lang==="ar"?"Bluetooth · المكالمات · DND · إعادة الجدولة بعد reboot · شاشة القفل: حالات Android يجب اختبارها على جهاز حقيقي.":"Bluetooth, calls, DND, reboot rescheduling and lock-screen behavior require real Android device validation."}</div>
  </SectionShell>;
}

function QuranCompanion({ lang, go }) {
  const [plan,setPlan]=useState("30");
  const [review,setReview]=useState(["الفاتحة ٥","الإخلاص ١"]);
  const [note,setNote]=useState("");
  return <SectionShell lang={lang} go={go} titleAr="رفيق القرآن" titleEn="Quran Companion" subtitleAr="استمرارية شخصية محلية: قراءة، استماع، حفظ وخطة بدون نقاط أو ضغط أو تقييم ديني." subtitleEn="Private local continuity across reading, listening, memorization and plans—without points, guilt or religiosity scoring." back="quran-platform">
    <div style={{ marginTop:24 }}><div style={{fontSize:10,opacity:.4,letterSpacing:".08em",fontWeight:600}}>{lang==="ar"?"الاستمرار":"CONTINUE"}</div><button onClick={()=>go("reader",{surahId:1})} style={{ marginTop:8, border:"none", background:"none", padding:0, cursor:"pointer", textAlign:lang==="ar"?"right":"left" }}><div className="font-editorial" style={{fontSize:24}}>{lang==="ar"?"الفاتحة · الآية ٣":"Al-Fātiḥah · Ayah 3"}</div><div style={{fontSize:11.5,opacity:.45,marginTop:5}}>{lang==="ar"?"موضع محلي تجريبي":"Local prototype position"}</div></button></div>
    <div style={{marginTop:28,borderTop:"1px solid rgba(16,16,15,.08)",paddingTop:18}}><div style={{fontSize:10,opacity:.4,fontWeight:600,letterSpacing:".08em"}}>{lang==="ar"?"خطة القراءة":"READING PLAN"}</div><div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>{["30","60","90","custom"].map(x=><ChoicePill key={x} active={plan===x} onClick={()=>setPlan(x)}>{x==="custom"?(lang==="ar"?"مخصص":"Custom"):(lang==="ar"?`${nDigits(x,lang)} يوم`:`${x} days`)}</ChoicePill>)}</div></div>
    <div style={{marginTop:26,borderTop:"1px solid rgba(16,16,15,.08)",paddingTop:18}}><div style={{fontSize:10,opacity:.4,fontWeight:600}}>{lang==="ar"?"مراجعة الحفظ":"MEMORIZATION REVIEW"}</div>{review.map((r,i)=><div key={r} style={{display:"flex",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid rgba(16,16,15,.06)"}}><span style={{fontSize:13.5}}>{r}</span><button onClick={()=>setReview(v=>v.filter((_,j)=>j!==i))} style={{border:"none",background:"none",fontSize:10.5,opacity:.5,cursor:"pointer"}}>{lang==="ar"?"مطمئن":"Comfortable"}</button></div>)}</div>
    <div style={{marginTop:24}}><div style={{fontSize:10,opacity:.4,fontWeight:600}}>{lang==="ar"?"ملاحظة خاصة":"PRIVATE NOTE"}</div><textarea value={note} onChange={e=>setNote(e.target.value)} placeholder={lang==="ar"?"ملاحظة شخصية — ليست تفسيراً":"Personal note — not Tafsir"} style={{width:"100%",marginTop:9,minHeight:88,border:"1px solid rgba(16,16,15,.12)",background:"transparent",borderRadius:10,padding:12,fontFamily:"inherit",resize:"vertical"}}/></div>
  </SectionShell>;
}

function FridayEidCenter({ lang, go }) {
  const [mode,setMode]=useState("friday");
  return <SectionShell lang={lang} go={go} titleAr="الجمعة والعيد" titleEn="Friday & Eid Moments" subtitleAr="Moments سياقية تظهر فقط عندما يكون التاريخ/الوقت موثوقاً، ولا تتحول إلى ثيمات زخرفية." subtitleEn="Contextual Moments appear only from trusted date/time data and never become decorative seasonal skins." back="calendar-zakat">
    <div style={{display:"flex",gap:8,marginTop:22}}><ChoicePill active={mode==="friday"} onClick={()=>setMode("friday")}>{lang==="ar"?"الجمعة":"Friday"}</ChoicePill><ChoicePill active={mode==="eid"} onClick={()=>setMode("eid")}>{lang==="ar"?"العيد":"Eid"}</ChoicePill></div>
    <div style={{marginTop:24}}>{mode==="friday"?<>
      <div className="font-editorial" style={{fontSize:26}}>{lang==="ar"?"لحظة الجمعة":"Friday Moment"}</div><div style={{fontSize:12.5,opacity:.5,marginTop:8,lineHeight:1.7}}>{lang==="ar"?"سورة الكهف، الصلاة على النبي ﷺ، بيانات الجمعة/المسجد عند توفر مصدر موثوق، ثم تختفي الحالة بعد انتهاء السياق.":"Surah Al-Kahf, salawat, verified Friday/mosque information when available, then the Moment recedes when context ends."}</div><button onClick={()=>go("quran-home")} style={{marginTop:18,border:"none",background:"none",padding:0,color:COLOR.goldDeep,cursor:"pointer"}}>{lang==="ar"?"فتح القرآن":"Open Quran"}</button>
    </>:<><div className="font-editorial" style={{fontSize:26}}>{lang==="ar"?"لحظة العيد":"Eid Moment"}</div><div style={{fontSize:12.5,opacity:.5,marginTop:8,lineHeight:1.7}}>{lang==="ar"?"صلاة العيد، التكبير والمصليات والزكاة ذات الصلة لا تظهر إلا من مصادر/تقويم موثوق، بدون بالونات أو gamification.":"Eid prayer, takbir, prayer grounds and related Zakat appear only from trusted providers/calendar data—without gamified seasonal decoration."}</div></>}</div>
  </SectionShell>;
}

function NamesCenter({ lang, go }) {
  const [saved,setSaved]=useState(false);
  return <SectionShell lang={lang} go={go} titleAr="أسماء الله الحسنى" titleEn="Names of Allah" subtitleAr="لا تُعرض قائمة افتراضية غير موثقة. النظام ينتظر Dataset مراجَعاً يوضح المنهج والمصدر والخلاف العلمي في الحصر." subtitleEn="No unsourced default list. The system awaits a reviewed dataset with methodology, provenance and scholarly context around enumeration." back="discover">
    <div style={{marginTop:28,padding:"22px 0",borderTop:"1px solid rgba(16,16,15,.08)",borderBottom:"1px solid rgba(16,16,15,.08)"}}><div style={{fontSize:10,opacity:.4,fontWeight:600}}>{lang==="ar"?"حالة المحتوى":"CONTENT STATE"}</div><div className="font-editorial" style={{fontSize:26,marginTop:8}}>{lang==="ar"?"بانتظار مصدر موثّق":"Trusted source required"}</div><div style={{fontSize:12,opacity:.48,lineHeight:1.7,marginTop:8}}>{lang==="ar"?"عند ربط المصدر: الاسم، المعنى، الدليل، الشرح، الصوت، الروابط، والمراجع.":"When connected: name, meaning, evidence, explanation, audio, related knowledge and references."}</div></div>
    <button onClick={()=>setSaved(v=>!v)} style={{marginTop:18,border:"none",background:"none",padding:0,color:saved?COLOR.goldDeep:COLOR.ink,cursor:"pointer"}}>{saved?(lang==="ar"?"تم حفظ اهتمامك":"Interest saved"):(lang==="ar"?"احفظ هذا القسم للرجوع":"Save this section")}</button>
  </SectionShell>;
}

function IslamicLibrary({ lang, go }) {
  const [filter,setFilter]=useState("books");
  const types=["books","khutbah","lessons","manuscripts"];
  return <SectionShell lang={lang} go={go} titleAr="المكتبة الإسلامية" titleEn="Islamic Library" subtitleAr="كتب وخطب ودروس ومخطوطات بمصدر وحقوق وإصدار واضح، مع بحث وملاحظات وتنزيل Offline." subtitleEn="Books, khutbahs, lessons and manuscripts with source, rights and version metadata, plus search, notes and offline packs." back="archive">
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:22}}>{types.map(x=><ChoicePill key={x} active={filter===x} onClick={()=>setFilter(x)}>{lang==="ar"?({books:"كتب",khutbah:"خطب",lessons:"دروس",manuscripts:"مخطوطات"}[x]):({books:"Books",khutbah:"Khutbahs",lessons:"Lessons",manuscripts:"Manuscripts"}[x])}</ChoicePill>)}</div>
    <div style={{marginTop:24,borderTop:"1px solid rgba(16,16,15,.08)",paddingTop:18}}><div className="font-editorial" style={{fontSize:23}}>{lang==="ar"?"المكتبة تنتظر مزودات مرخّصة":"Library awaits licensed providers"}</div><div style={{fontSize:12,opacity:.46,marginTop:7,lineHeight:1.7}}>{lang==="ar"?"المعاينة لا تنشئ كتباً أو خطباً من الذاكرة. عند الربط تظهر بيانات المصدر وحقوق الاستخدام والتنزيل.":"The prototype does not invent books or sermons. Connected content will expose provenance, usage rights and offline availability."}</div></div>
  </SectionShell>;
}

function QiblaMap({ lang, go }) {
  const [bearing,setBearing]=useState(198);
  return <SectionShell lang={lang} go={go} titleAr="القبلة — الخريطة" titleEn="Qibla Map" subtitleAr="Fallback عندما تكون حساسات الجهاز ضعيفة: اتجاه رقمي وخط على الخريطة من الموقع إلى مكة. الرقم هنا محاكاة واجهة فقط." subtitleEn="Fallback for unreliable sensors: numeric bearing and a map line from user location to Makkah. The number here is UI simulation only." back="prayer">
    <div style={{marginTop:28,display:"flex",alignItems:"center",justifyContent:"center",height:230,position:"relative",border:"1px solid rgba(16,16,15,.08)",borderRadius:18,overflow:"hidden"}}><div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(23,59,87,.06),rgba(181,154,98,.08))"}}/><div style={{position:"relative",textAlign:"center"}}><Navigation2 size={28} strokeWidth={1.3}/><div style={{fontSize:42,fontFamily:"Fraunces,serif",marginTop:10}}>{nDigits(bearing,lang)}°</div><div style={{fontSize:11,opacity:.45}}>{lang==="ar"?"محاكاة — ليس قياساً فعلياً":"Simulation — not a live measurement"}</div></div></div>
    <input type="range" min="0" max="359" value={bearing} onChange={e=>setBearing(Number(e.target.value))} className="sk-slider" style={{width:"100%",marginTop:22}}/>
    <div style={{fontSize:11.5,opacity:.48,lineHeight:1.7,marginTop:12}}>{lang==="ar"?"الإنتاج: GPS أو موقع يدوي → geodesic bearing → Map provider موثوق. لا حاجة لتتبع دائم.":"Production: GPS/manual location → geodesic bearing → trusted map provider. No continuous tracking required."}</div>
  </SectionShell>;
}

function DownloadManager({ lang, go }) {
  const packs=[
    ["quran","القرآن الأساسي","Essential Quran","48 MB"],["audio","تلاوة مختارة","Selected recitation","320 MB"],["tafsir","تفسير","Tafsir pack","95 MB"],["hajj","الحج والعمرة للطوارئ","Emergency Hajj/Umrah","34 MB"],["adhkar","الأذكار","Adhkar","6 MB"]
  ];
  const [installed,setInstalled]=useState(new Set(["quran"]));
  const toggle=id=>setInstalled(s=>{const n=new Set(s);n.has(id)?n.delete(id):n.add(id);return n;});
  return <SectionShell lang={lang} go={go} titleAr="إدارة التنزيلات" titleEn="Download Manager" subtitleAr="إدارة موحدة للحزم مع الحجم والحالة، بدون تنزيل صامت أو استهلاك مساحة غير واضح." subtitleEn="Unified content-pack management with size and state—no silent large downloads." back="offline-sync">
    <div style={{marginTop:22}}>{packs.map(([id,ar,en,size],i)=><div key={id} style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",padding:"15px 0",borderTop:"1px solid rgba(16,16,15,.08)"}}><div><div style={{fontSize:14.5,fontWeight:i===0?600:500}}>{lang==="ar"?ar:en}</div><div style={{fontSize:11,opacity:.42,marginTop:3}}>{size} · {installed.has(id)?(lang==="ar"?"مثبّت محلياً":"Installed locally"):(lang==="ar"?"غير مثبّت":"Not installed")}</div></div><button onClick={()=>toggle(id)} style={{border:"none",background:"none",cursor:"pointer",color:installed.has(id)?COLOR.terracotta:COLOR.goldDeep,fontSize:11.5}}>{installed.has(id)?(lang==="ar"?"إزالة":"Remove"):(lang==="ar"?"إضافة تجريبية":"Add demo")}</button></div>)}</div>
    <div style={{marginTop:20,fontSize:11.5,opacity:.45,lineHeight:1.7}}>{lang==="ar"?"التنزيل الحقيقي يحتاج مزود محتوى، تحقق سلامة، resume/retry، ومساحة تخزين فعلية. الحالات هنا Prototype state فقط.":"Real downloads require a provider, integrity checks, resume/retry and actual storage. These are prototype states only."}</div>
  </SectionShell>;
}

function GuestPrivacyCenter({ lang, go }) {
  const [guest,setGuest]=useState(false), [ai,setAi]=useState("local"), [screenshots,setScreenshots]=useState(true);
  return <SectionShell lang={lang} go={go} titleAr="الضيف والخصوصية" titleEn="Guest & Privacy Modes" subtitleAr="تحكم صريح بما يراه الضيف، وأين تعمل قدرات AI، وحماية المناطق الخاصة." subtitleEn="Explicit control over guest visibility, AI processing mode and private-surface protection." back="me-privacy">
    <div style={{marginTop:22,display:"flex",justifyContent:"space-between",padding:"15px 0",borderTop:"1px solid rgba(16,16,15,.08)"}}><div><div style={{fontSize:14}}>{lang==="ar"?"وضع الضيف":"Guest Mode"}</div><div style={{fontSize:11,opacity:.42,marginTop:3}}>{lang==="ar"?"يخفي السجل والعائلة والملاحظات":"Hides history, family and notes"}</div></div><Switch on={guest} onChange={()=>setGuest(v=>!v)}/></div>
    <div style={{padding:"15px 0",borderTop:"1px solid rgba(16,16,15,.08)"}}><div style={{fontSize:14,marginBottom:10}}>{lang==="ar"?"وضع الذكاء":"AI Mode"}</div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{["off","local","trusted"].map(x=><ChoicePill key={x} active={ai===x} onClick={()=>setAi(x)}>{lang==="ar"?({off:"إيقاف",local:"محلي",trusted:"سحابة موثوقة"}[x]):({off:"Off",local:"Local",trusted:"Trusted cloud"}[x])}</ChoicePill>)}</div></div>
    <div style={{display:"flex",justifyContent:"space-between",padding:"15px 0",borderTop:"1px solid rgba(16,16,15,.08)"}}><div><div style={{fontSize:14}}>{lang==="ar"?"السماح بلقطات الشاشة في المناطق الخاصة":"Screenshots in private areas"}</div><div style={{fontSize:11,opacity:.42,marginTop:3}}>{lang==="ar"?"للملاحظات والتسجيلات والعائلة":"Notes, recordings and family"}</div></div><Switch on={screenshots} onChange={()=>setScreenshots(v=>!v)}/></div>
  </SectionShell>;
}

function CorrectionUpdatesCenter({ lang, go }) {
  const [report,setReport]=useState(""), [sent,setSent]=useState(false);
  return <SectionShell lang={lang} go={go} titleAr="التصحيحات والتحديثات" titleEn="Corrections & Updates" subtitleAr="Feedback loop للمحتوى الديني: الإبلاغ لا يعدّل المصدر مباشرة، بل يدخل مسار مراجعة وتدقيق وإصدار." subtitleEn="Religious-content feedback enters review/versioning; user reports never directly overwrite source data." back="verification">
    <div style={{marginTop:22}}><div style={{fontSize:10,opacity:.4,fontWeight:600}}>{lang==="ar"?"إبلاغ عن مشكلة":"REPORT AN ISSUE"}</div><textarea value={report} onChange={e=>{setReport(e.target.value);setSent(false)}} placeholder={lang==="ar"?"صف المشكلة أو المرجع…":"Describe the issue or reference…"} style={{width:"100%",minHeight:100,marginTop:10,border:"1px solid rgba(16,16,15,.12)",background:"transparent",borderRadius:10,padding:12,fontFamily:"inherit"}}/><button disabled={!report.trim()} onClick={()=>setSent(true)} style={{marginTop:10,border:"none",background:"none",padding:0,color:report.trim()?COLOR.goldDeep:COLOR.ink,opacity:report.trim()?1:.3,cursor:report.trim()?"pointer":"default"}}>{sent?(lang==="ar"?"أُضيف لمسار المراجعة (محاكاة)":"Added to review queue (simulation)"):(lang==="ar"?"إرسال للمراجعة":"Send for review")}</button></div>
    <div style={{marginTop:28,borderTop:"1px solid rgba(16,16,15,.08)",paddingTop:18}}><div style={{fontSize:10,opacity:.4,fontWeight:600}}>{lang==="ar"?"سجل التحديث":"UPDATE HISTORY"}</div><div style={{fontSize:13.5,marginTop:10}}>{lang==="ar"?"لا توجد حزم خارجية متصلة في هذه المعاينة":"No external religious packs are connected in this preview"}</div><div style={{fontSize:11,opacity:.45,marginTop:5,lineHeight:1.6}}>{lang==="ar"?"الإنتاج يعرض الإصدار، المصدر، سبب التصحيح، checksum والتوقيع.":"Production exposes version, source, correction reason, checksum and signature."}</div></div>
  </SectionShell>;
}

function TeacherImamCenter({ lang, go }) {
  const [role,setRole]=useState("teacher");
  return <SectionShell lang={lang} go={go} titleAr="المعلم والإمام" titleEn="Teacher & Imam" subtitleAr="مساحات خاصة للمعلّم أو الجهة الموثقة، منفصلة عن تجربة المستخدم العادي." subtitleEn="Private role-based surfaces for teachers or verified mosque administration, separate from ordinary users." back="community">
    <div style={{display:"flex",gap:8,marginTop:22}}><ChoicePill active={role==="teacher"} onClick={()=>setRole("teacher")}>{lang==="ar"?"المعلم":"Teacher"}</ChoicePill><ChoicePill active={role==="imam"} onClick={()=>setRole("imam")}>{lang==="ar"?"الإمام/المسجد":"Imam/Mosque"}</ChoicePill></div>
    {role==="teacher"?<div style={{marginTop:22}}>{[["واجب آيات","Ayah assignment"],["تسجيل طالب اختياري","Optional student recording"],["حلقة خاصة","Private circle"],["ملاحظات تعليمية","Teaching notes"]].map(([ar,en],i)=><Row key={ar} lang={lang} first={i===0} label={lang==="ar"?ar:en} sub={lang==="ar"?"يتطلب backend خاص وصلاحيات":"Requires private backend and permissions"}/>)}</div>:<div style={{marginTop:22}}>{[["تحديث الإقامة","Update Iqamah"],["إعلان درس","Publish lesson"],["بيانات الجمعة","Friday information"],["إدارة QR رسمي","Manage official QR"]].map(([ar,en],i)=><Row key={ar} lang={lang} first={i===0} label={lang==="ar"?ar:en} sub={lang==="ar"?"يتطلب توثيق هوية المسجد":"Requires verified mosque identity"}/>)}</div>}
  </SectionShell>;
}

function PublicDisplayCenter({ lang, go }) {
  const [mode,setMode]=useState("home");
  return <SectionShell lang={lang} go={go} titleAr="الشاشة المنزلية والعامة" titleEn="Home & Public Display" subtitleAr="واجهة مستقلة للبيت أو المسجد: لا حساب شخصي ولا بيانات خاصة." subtitleEn="A dedicated home/mosque display surface with no personal account or private activity." back="devices">
    <div style={{display:"flex",gap:8,marginTop:20}}><ChoicePill active={mode==="home"} onClick={()=>setMode("home")}>{lang==="ar"?"البيت":"Home"}</ChoicePill><ChoicePill active={mode==="mosque"} onClick={()=>setMode("mosque")}>{lang==="ar"?"المسجد":"Mosque"}</ChoicePill></div>
    <div style={{marginTop:26,minHeight:270,borderRadius:18,background:`linear-gradient(180deg,${STAGES[2].from},${STAGES[2].to})`,padding:22,display:"flex",flexDirection:"column",justifyContent:"space-between"}}><div><div style={{fontSize:10,opacity:.45}}>{mode==="home"?(lang==="ar"?"سكينة في المنزل":"SAKINAH HOME"):(lang==="ar"?"شاشة المسجد":"MOSQUE DISPLAY")}</div><div className="font-editorial" style={{fontSize:28,marginTop:8}}>{lang==="ar"?"الظهر":"Dhuhr"} · 13:08</div></div><DayArc hourNow={12.4} lang={lang} compact isDark={false}/><div style={{fontSize:11.5,opacity:.5}}>{mode==="mosque"?(lang==="ar"?"الإقامة تحتاج مصدر مسجد موثّق":"Iqamah requires verified mosque data"):(lang==="ar"?"القرآن · التاريخ · الصلاة القادمة":"Quran · date · next prayer")}</div></div>
  </SectionShell>;
}
...
export default function SakinahApp() {
  ...
  const hourNow = preview !== null ? preview : now.getHours() + now.getMinutes() / 60;
  ...
  const screens = {
    ...
    "reciters": () => <ReciterLibrary lang={lang} go={go} />,
    ...
    "guest-privacy": () => <GuestPrivacyCenter lang={lang} go={go} />,
    "corrections": () => <CorrectionUpdatesCenter lang={lang} go={go} />,
