...
export default function SakinahNativeReadyLayer(){
 ...
 return <div style={{minHeight:"100vh"}}>
  ...
  {!tool&&<div style={{position:"fixed",left:12,bottom:144,zIndex:13000,display:"grid",gap:7}}><button onClick={()=>setTool("alerts")} title={lang==="ar"?"المؤذن والتنبيهات":"Adhan & Alerts"} style={{width:48,height:48,borderRadius:17,border:"1px solid rgba(16,16,15,.08)",background:"linear-gradient(145deg,#173B57,#0C293E)",color:"#E7D29B",boxShadow:"0 10px 28px rgba(23,59,87,.22)",fontSize:19}}>◔</button><button onClick={()=>setTool("adhan-audio")} title={lang==="ar"?"أصوات الأذان":"Adhan Sounds"} style={{width:48,height:48,borderRadius:17,border:"1px solid rgba(16,16,15,.08)",background:"rgba(246,243,236,.96)",color:C.gold,boxShadow:"0 8px 22px rgba(0,0,0,.10)",fontSize:18}}>♪</button><button onClick={()=>setTool("widget")} title="Widget" style={{width:48,height:48,borderRadius:17,border:"1px solid rgba(16,16,15,.08)",background:"rgba(246,243,236,.96)",color:C.lapis,boxShadow:"0 8px 22px rgba(0,0,0,.10)",fontSize:18}}>▦</button></div>}
 </div>;
}
