...
function ensureMarker() {
  ...
  Object.assign(marker.style, {
    position: 'fixed', zIndex: '20', width: '26px', height: '26px', display: 'grid',
    placeItems: 'center', pointerEvents: 'none', transform: 'translate(-50%,-50%)',
    transition: 'left .3s ease, top .3s ease, opacity .2s ease',
    fontFamily: 'Georgia, serif', fontSize: '21px', lineHeight: '1'
  });
...
function updateMarker() {
  ...
  if (!hero || !svg) { marker.style.display = 'none'; return; }

  const rect = svg.getBoundingClientRect();
  const heroRect = hero.getBoundingClientRect();
  if (!rect.width || !rect.height || rect.bottom < heroRect.top || rect.top > heroRect.bottom) {
  ...
  marker.style.textShadow = day ? '0 0 10px rgba(205,166,78,.30)' : '0 0 10px rgba(240,233,215,.28)';
}
  ...
  const start = () => {
    ...
    document.addEventListener('input', e => {
      if (e.target?.matches?.('input[type="range"][aria-label="معاينة وقت الهيرو"]')) {
        e.target.dataset.celestialPreview = '1'; requestAnimationFrame(updateMarker);
      }
    }, true);
