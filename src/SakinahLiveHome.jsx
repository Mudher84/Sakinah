...
export default function SakinahLiveHome({profileAvatar="",profileName="أنا",onOpenProfile}){
 ...
 return <div className="mm-reference-home" dir="rtl">
  <style>{`
   ...
   .mm-service-groups{margin:24px 20px 0;display:grid;gap:10px}.mm-service-group{border:1px solid rgba(14,42,63,.08);border-radius:18px;background:rgba(255,255,255,.28);overflow:hidden}.mm-service-head{width:100%;border:0;background:transparent;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:13px 14px;cursor:pointer;text-align:right}.mm-service-title{font-size:12.5px;line-height:1.4}.mm-service-toggle{width:24px;height:24px;flex:0 0 24px;border-radius:50%;display:grid;place-items:center;background:rgba(212,168,78,.10);color:${C.gold};font-size:15px;line-height:1}.mm-service-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:0 10px 10px}.mm-service-card{min-height:82px;border:1px solid rgba(14,42,63,.08);border-radius:15px;background:rgba(255,255,255,.52);padding:10px 8px;cursor:pointer}.mm-service-icon{font-size:18px;color:${C.gold}}.mm-service-name{font-size:10px;margin-top:7px;line-height:1.55}
   .mm-ref-tap{transition:transform .14s ease}.mm-ref-tap:active{transform:scale(.97)}
   @media(max-width:520px){.mm-prayer-hero{padding:16px 14px 18px;border-radius:0 0 30px 30px}.mm-time-panel{margin-top:27px}.mm-arc-wrap{height:112px}.mm-next-name{font-size:27px}.mm-next-name:before,.mm-next-name:after{width:22px;margin:0 8px}.mm-service-groups{margin:20px 14px 0;gap:8px}.mm-service-head{padding:12px}.mm-service-grid{gap:7px;padding:0 8px 8px}.mm-service-card{min-height:78px}}
  `}</style>
  <div className="mm-reference-frame"><div className="mm-reference-sheet">
   ...
   <section className="mm-service-groups">{GROUPS.map((g,gi)=><div key={g.name} className="mm-service-group"><button className="mm-service-head" onClick={()=>setOpen(v=>v.map((x,i)=>i===gi?!x:x))}><span className="mm-cairo mm-service-title">{g.name}</span><span className="mm-service-toggle" aria-hidden="true">{open[gi]?"−":"+"}</span></button>{open[gi]&&<div className="mm-service-grid">{g.items.map(([id,icon,title])=><button key={id} className="mm-ref-tap mm-service-card" onClick={()=>emit(id)}><div className="mm-service-icon">{icon}</div><div className="mm-cairo mm-service-name">{title}</div></button>)}</div>}</div>)}</section>

   <section style={{margin:"22px 20px 30px",padding:"18px",borderRadius:22,background:"linear-gradient(135deg,#183B52,#102D43)",color:C.bg}}><div style={{fontSize:11,color:"rgba(247,243,234,.6)"}}>المسجد القريب</div><div className="mm-amiri" style={{fontSize:22,marginTop:7}}>وضع المسجد</div><div style={{fontSize:10,color:"rgba(247,243,234,.48)",marginTop:5}}>فعّل الواجهة الهادئة أثناء وجودك في المسجد.</div><button className="mm-ref-tap" onClick={()=>setMasjid(v=>!v)} style={{marginTop:13,border:`1px solid ${masjid?C.gold:"rgba(247,243,234,.18)"}`,borderRadius:999,padding:"8px 14px",background:masjid?"rgba(192,160,98,.16)":"transparent",color:masjid?C.gold:C.bg,cursor:"pointer",fontSize:10}}>{masjid?"مفعّل":"تفعيل"}</button></section>
  </div></div>
 </div>;
}
