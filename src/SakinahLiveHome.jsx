...
function stageFor(t,previewHour=null){
 const now=new Date(),x=previewHour==null?now.getHours()+now.getMinutes()/60:previewHour;
...
export default function SakinahLiveHome(){
 ...
 const next=useMemo(()=>nextPrayer(data?.timings),[data,tick]);const stage=stageFor(data?.timings,preview);const fg=stage.dark?C.ivory:C.ink;const date=new Intl.DateTimeFormat("ar-IQ",{weekday:"long",day:"numeric",month:"long"}).format(new Date());const hijri=data?.date?.hijri?.date?`${data.date.hijri.day} ${data.date.hijri.month?.ar||""} ${data.date.hijri.year}`:"";
 return <div className="sakinah-live-home" style={{position:"relative",display:"block",isolation:"isolate",minHeight:"100vh",width:"100%",overflowX:"hidden",background:C.ivory,color:C.ink,paddingBottom:118}} dir="rtl">
  <style>{`@keyframes sakinahWordPulse{0%,100%{opacity:.72;transform:scale(1);text-shadow:0 0 0 rgba(184,34,38,0)}50%{opacity:1;transform:scale(1.07);text-shadow:0 0 12px rgba(184,34,38,.28)}}.sakinah-wordmark{display:inline-block;color:#B82226!important;opacity:1!important;transform-origin:center;animation:sakinahWordPulse 2.2s ease-in-out infinite}@media (prefers-reduced-motion:reduce){.sakinah-wordmark{animation:none}}`}</style>
  <section className="sakinah-live-hero" style={{position:"relative",zIndex:2,display:"block",boxSizing:"border-box",width:"100%",minHeight:590,padding:"18px 20px 22px",background:`radial-gradient(circle at 74% 10%,${stage.glow},transparent 34%),linear-gradient(180deg,${stage.from} 0%,${stage.via} 55%,${stage.to} 100%)`,color:fg,transition:"background 1.4s ease,color .7s ease"}}>
   <div style={{marginBottom:14,padding:"8px 10px 9px",borderRadius:14,border:`1px solid ${stage.dark?"rgba(246,243,236,.14)":"rgba(16,16,15,.08)"}`,background:stage.dark?"rgba(255,255,255,.05)":"rgba(255,255,255,.28)",backdropFilter:"blur(10px)"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:5}}><span style={{fontSize:9.5,opacity:.48}}>معاينة الوقت · {preview==null?"الآن":stageLabel(stage.id)}</span><button onClick={()=>setPreview(null)} style={{border:0,borderRadius:9,padding:"5px 9px",background:preview==null?"rgba(181,154,98,.16)":"transparent",color:fg,fontFamily:"inherit",fontSize:9.5}}>الآن</button></div><input aria-label="معاينة وقت الهيرو" type="range" min="0" max="23.75" step="0.25" value={preview==null?(new Date().getHours()+new Date().getMinutes()/60):preview} onChange={e=>setPreview(Number(e.target.value))} style={{width:"100%",accentColor:C.gold}}/></div>
   <header style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}><div><div className="sakinah-wordmark" style={{fontSize:9,letterSpacing:1.4}}>SAKINAH</div><div style={{fontFamily:"Fraunces,serif",fontSize:29,marginTop:2}}>سكينة</div><div style={{fontSize:10,opacity:.46,marginTop:5}}>{date}{hijri?` · ${hijri}`:""}</div></div><button onClick={load} style={{width:42,height:42,borderRadius:15,border:`1px solid ${stage.dark?"rgba(246,243,236,.18)":"rgba(16,16,15,.08)"}`,background:stage.dark?"rgba(255,255,255,.07)":"rgba(255,255,255,.38)",color:fg,fontSize:17}}>↻</button></header>
   <div style={{marginTop:70}}><div style={{fontSize:11,letterSpacing:1,opacity:.52}}>{next?AR[next.id]:"الصلاة القادمة"}</div><div style={{fontSize:64,lineHeight:1,fontWeight:600,marginTop:9,fontFamily:"'IBM Plex Sans Arabic',sans-serif"}}>{next?.time||"--:--"}</div><div style={{fontSize:12.5,opacity:.56,marginTop:8}}>متبقي · {next?remaining(next.at-tick):"بانتظار المواقيت"}</div></div>
