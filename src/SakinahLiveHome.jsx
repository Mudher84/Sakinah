...
function TodayScreen({ lang, stage, hourNow, nextPrayer, remH, remM, moment, go, onScrub, lastRead, qiblaDeg, prayerTimes }) {
  ...
  const [openLibrary, setOpenLibrary] = useState("worship");
  const [mosqueLocation, setMosqueLocation] = useState(false);
  ...
  const useDarkText = hourNow >= prayerTimes.sunrise && hourNow < prayerTimes.asr;
  const textTheme = {
    "--sk-text": useDarkText ? "#183246" : "#fffdf8",
    "--sk-muted": useDarkText ? "rgba(24,50,70,.82)" : "rgba(255,253,248,.88)",
    "--sk-soft": useDarkText ? "rgba(24,50,70,.64)" : "rgba(255,253,248,.76)",
    "--sk-line": useDarkText ? "rgba(24,50,70,.24)" : "rgba(255,255,255,.30)",
  };
  ...
  const displayTime = (hour) => {
    ...
    return <>{`${String(twelve).padStart(2, "0")}:${minutes}`}<small className="sk-time-marker">{h < 12 ? "AM" : "PM"}</small></>;
  };
  ...
  return <div dir="rtl" className="sk-warm-home" style={{...textTheme,background:warmSky[1],transition:"background 1.2s linear"}}>
    <style>{`
      ...
      .sk-library{margin:6px 20px 34px;padding-top:12px;border-top:1px solid var(--sk-line)}.sk-library-head{display:flex;align-items:center;justify-content:space-between;margin:0 2px 12px}.sk-library-head h2{margin:0;font-size:18px;font-weight:600;color:var(--sk-text)}.sk-library-head span{font-size:11px;color:var(--sk-muted)}.sk-library-group{border:1px solid var(--sk-line);border-radius:22px;background:rgba(255,255,255,.10);margin-bottom:10px;overflow:hidden}.sk-library-trigger{display:flex;width:100%;align-items:center;justify-content:space-between;border:0;background:none;color:var(--sk-text);padding:15px 18px;cursor:pointer;font:inherit}.sk-library-trigger strong{font-size:16px;font-weight:500}.sk-library-trigger small{margin-right:7px;color:var(--sk-muted);font-size:11px}.sk-library-trigger i{font-style:normal;color:#efd093;font-size:16px}.sk-library-services{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:0 12px 14px}.sk-library-service{min-height:82px;border:0;border-radius:17px;background:rgba(255,255,255,.12);color:var(--sk-text);font:inherit;cursor:pointer}.sk-library-service em{display:block;height:29px;padding-top:11px;font-style:normal;color:#f0cb8d;font-size:18px}.sk-library-service span{display:block;font-size:12px}.sk-mosque-card{display:flex;align-items:center;justify-content:space-between;margin-top:26px;padding:19px 18px;border-radius:23px;background:rgba(12,45,69,.92);color:#fffaf2}.sk-mosque-card h3{margin:0 0 4px;font-size:17px;font-weight:600}.sk-mosque-card p{margin:0;color:rgba(255,255,255,.72);font-size:11px}.sk-switch{width:46px;height:28px;padding:3px;border:0;border-radius:999px;background:rgba(255,255,255,.28);cursor:pointer;transition:.2s}.sk-switch i{display:block;width:22px;height:22px;border-radius:50%;background:#fffaf2;transition:.2s}.sk-switch.on{background:#d9b47c}.sk-switch.on i{transform:translateX(-18px)}
      .sk-home-discover{margin:0 20px 18px;padding:18px 0;border-top:1px solid var(--sk-line)}.sk-discover-ayah{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;margin-bottom:18px}.sk-discover-ayah label{font-size:11px;color:var(--sk-soft)}.sk-discover-ayah p{grid-column:1/-1;margin:0;font-family:'Amiri Quran','Amiri',serif;font-size:25px;line-height:1.4;color:var(--sk-text);text-align:center}.sk-discover-ayah span{font-size:11px;color:var(--sk-muted)}.sk-discover-actions{display:flex;gap:16px;color:#e3be82;font-size:17px}.sk-discover-search{display:flex;align-items:center;gap:8px;border:1px solid var(--sk-line);border-radius:18px;background:rgba(255,255,255,.14);padding:0 15px;margin-bottom:23px}.sk-discover-search input{width:100%;height:54px;border:0;outline:0;background:transparent;color:var(--sk-text);font:inherit;font-size:13px}.sk-discover-search input::placeholder{color:var(--sk-muted)}.sk-discover-search i{font-style:normal;color:var(--sk-muted)}.sk-discover-title{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.sk-discover-title b{font-size:17px;font-weight:600;color:var(--sk-text)}.sk-discover-title span{font-size:11px;color:var(--sk-muted)}.sk-frequent-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.sk-frequent-card{position:relative;min-height:80px;padding:14px 15px;border:1px solid var(--sk-line);border-radius:20px;background:rgba(255,255,255,.12);color:var(--sk-text);text-align:right;cursor:pointer;font:inherit}.sk-frequent-card b{display:block;font-size:15px;font-weight:500}.sk-frequent-card small{display:block;margin-top:5px;color:var(--sk-muted);font-size:11px}.sk-frequent-card em{position:absolute;inset-inline-end:14px;top:19px;width:31px;height:31px;border-radius:12px;background:rgba(240,213,164,.22);color:#e7c486;font-style:normal;text-align:center;line-height:31px}
    `}</style>
    <main className="sk-warm-card" style={{position:"relative",background:`linear-gradient(178deg,${warmSky[0]} 0%,${warmSky[1]} 42%,${warmSky[2]} 100%)`,transition:"background 1.2s linear"}}>
      ...
      <section className="sk-home-discover"><div className="sk-discover-ayah"><label>آية اليوم</label><div className="sk-discover-actions"><span>▷</span><span>♡</span><span>↗</span></div><p>وَعَصَىٰ آدَمُ رَبَّهُ فَغَوَىٰ</p><span>طه · الآية ١٢١</span></div><label className="sk-discover-search"><i>⌕</i><input value={homeSearch} onChange={(e) => setHomeSearch(e.target.value)} placeholder="ابحث في الخدمات، السور، الأذكار…" aria-label="ابحث في التطبيق" /></label><div className="sk-discover-title"><b>الأكثر استخداماً</b><span>يتعلّم من عاداتك</span></div><div className="sk-frequent-grid"><button className="sk-frequent-card" onClick={() => go("quran-home")}><em>◉</em><b>القرآن</b><small>الكهف · ٩:١٢</small></button><button className="sk-frequent-card" onClick={() => go("adhkar-home")}><em>✧</em><b>الأذكار</b><small>المساء</small></button><button className="sk-frequent-card" onClick={() => go("prayer")}><em>◈</em><b>التسبيح</b><small>٣٧ / ١٠٠</small></button><button className="sk-frequent-card" onClick={() => go("adhkar-home")}><em>♪</em><b>الأناشيد</b><small>٨ ملفات</small></button></div></section>
      <section className="sk-library" aria-label="المكتبة"><div className="sk-library-head"><h2>المكتبة</h2><span>١٨ خدمة</span></div>{[
        ["worship", "عبادات", "٦ خدمات", [["◉", "الصلاة والوضوء"], ["☾", "الصيام"], ["✧", "المناسك"], ["○", "رمضان"], ["◍", "الجمعة"], ["$", "الزكاة"]]],
        ["learn", "تعلّم وحفظ", "٦ خدمات", [["◈", "القرآن"], ["◌", "الأذكار"], ["✦", "التجويد"]]],
        ["tools", "أدوات", "٣ خدمات", [["◒", "القبلة"], ["⌁", "المسبحة"], ["◉", "البطاقات"]]],
        ["kids", "للأطفال", "٣ خدمات", [["★", "المعلّم"], ["◒", "قصص"], ["♪", "أناشيد"]]],
      ].map(([id, title, count, services]) => <div key={id} className="sk-library-group"><button className="sk-library-trigger" onClick={() => setOpenLibrary(openLibrary === id ? "" : id)}><span><strong>{title}</strong><small>{count}</small></span><i>{openLibrary === id ? "⌃" : "⌄"}</i></button>{openLibrary === id && <div className="sk-library-services">{services.map(([icon, label]) => <button className="sk-library-service" key={label}><em>{icon}</em><span>{label}</span></button>)}</div>}</div>)}<div className="sk-mosque-card"><div><h3>موضع المسجد</h3><p>يكتم الإشعارات تلقائياً حتى تنتهي الصلاة</p></div><button className={`sk-switch ${mosqueLocation ? "on" : ""}`} onClick={() => setMosqueLocation((value) => !value)} aria-label="تفعيل موضع المسجد"><i /></button></div></section>
    </main>
  </div>;
}
